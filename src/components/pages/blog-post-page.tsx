...
export default function BlogPostPage({ slug }: { slug: string }) {
  ...
  if (!post) return (<div className="section-padding bg-white"><div className="max-w-3xl mx-auto px-4 text-center py-16"><h2 className="text-2xl font-bold mb-4">Post Not Found</h2><p className="text-muted-foreground mb-6">The blog post you are looking for does not exist.</p><Button asChild><Link href="/blog"><ArrowLeft className="w-4 h-4 mr-2" />Back to Blog</Link></Button></div></div>);
  ...
  return (
    <div className="section-padding bg-white">
      <article className="max-w-3xl mx-auto px-4">
        <Link href="/blog" className="inline-flex items-center gap-1 text-rose-dark hover:underline text-sm mb-6"><ArrowLeft className="w-4 h-4" />Back to Blog</Link>
        ...
        <div className="mt-8 pt-8 border-t flex items-center justify-between">
          <Link href="/blog" className="text-rose-dark font-semibold hover:underline inline-flex items-center gap-1"><ArrowLeft className="w-4 h-4" />All Posts</Link>
